...
// see the credit further down), generated by a one-off script, not
// checked-in source photography. Real per-combination product photos
// replace these one file at a time later: same filename, real content —
// no code change needed when that happens.
function productImagePath(gender: Gender, model: ModelKey, colorNameKey: string): string {
...
// Future option (not wired up yet): each model could carry its own
// male/female photo pair, so picking a model would also swap the center
// image the way the gender toggle does today — HERO_IMAGES would become
// keyed by [model][gender] once those photos exist.
const MODELS = [
...
// of the app's light/dark theme (unlike every other color in this project,
// which comes from app/globals.css's @theme tokens per CLAUDE.md section
// 7). Placeholder sets of commonly available scrub colors, split by
// gender since the real catalogs differ; the client will give the real
// lineup later, and swapping these is a one-line edit per row.
type PaletteColor = { hex: string; nameKey: string };
const PALETTE: Record<Gender, PaletteColor[]> = {
  male: [
...
type CityKey = "LA_PAZ" | "SANTA_CRUZ";
...
export function LandingHero() {
  ...
  const quoteSummary = {
    gender: t(gender === "male" ? "genderMale" : "genderFemale"),
    model: t(MODELS.find((model) => model.key === selectedModel)!.nameKey),
    color: t(selectedColor),
    size: selectedSize ?? t("quoteSizeUnspecified"),
  };
...
// No backend yet for order/quote requests — that's a future session's
// work (a real DB table with a pending/attended/converted-to-sale status,
// per the plan). Until then this hands the request straight to WhatsApp
// with everything prefilled, using the same native-<dialog> +
// framer-motion-content pattern as PromptModal
// (components/ui/prompt-modal.tsx) — see
// .claude/skills/motion-and-transitions/SKILL.md.
function QuoteRequestModal({
  ...
  summary: { gender: string; model: string; color: string; size: string };
}) {
  const t = useTranslations("Landing");
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const lines = [
      t("quoteGreeting", { name }),
      "",
      t("quoteIntro"),
      `${t("quoteFieldGender")}: ${summary.gender}`,
      `${t("quoteFieldModel")}: ${summary.model}`,
      `${t("quoteFieldColor")}: ${summary.color}`,
      `${t("quoteFieldSize")}: ${summary.size}`,
    ];
    if (note.trim()) {
      lines.push("", `${t("quoteFieldNote")}: ${note.trim()}`);
    }
    lines.push("", `${t("quoteFieldPhone")}: ${phone}`);
  ...
  return (
    <dialog
      ...
      className="fixed top-1/2 left-1/2 m-0 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-md border border-border bg-background p-0 text-foreground backdrop:bg-black/50"
    >
      <motion.div
        initial={false}
      ...
      >
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-6">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-sm text-muted-foreground">{t("quoteDescription")}</p>
          </div>

          <div className="flex flex-wrap gap-x-4 gap-y-1 rounded-md border border-border bg-muted/40 p-3 text-sm">
            <span>
              <span className="text-muted-foreground">{t("quoteFieldGender")}: </span>
              {summary.gender}
            </span>
            <span>
              <span className="text-muted-foreground">{t("quoteFieldModel")}: </span>
              {summary.model}
            </span>
            <span>
              <span className="text-muted-foreground">{t("quoteFieldColor")}: </span>
              {summary.color}
            </span>
            <span>
              <span className="text-muted-foreground">{t("quoteFieldSize")}: </span>
              {summary.size}
            </span>
          </div>

          <FormField label={t("quoteNameLabel")} htmlFor="quote-name">
            <Input
              id="quote-name"
              required
              value={name}
          ...
          <FormField label={t("quotePhoneLabel")} htmlFor="quote-phone">
            <Input
              id="quote-phone"
              type="tel"
              required
          ...
          <FormField label={t("quoteNoteLabel")} htmlFor="quote-note">
            <Textarea
              id="quote-note"
              rows={3}
              value={note}
